...
const AddItemsDialog: FunctionComponent<AddItemsDialogProps> = (props: AddItemsDialogProps) => {
    ...
    const {
        inventory = [],
        weapons = []
    } = useTypedSelector(state => state.firebase.data?.addItemsDialog?.[character]) ?? {};
    ...
    const addSelectedItemsToCharacter = async () => {
        ...
        for (let selectedKey in selection) {
            ...
            newKeys.push(newKey);
            await itemsRef.child(newKey).set(selection[selectedKey]);
        }
        ...
        await firebase.ref(`/characters/${character}/weapons`)
            .set(newWeapons);
