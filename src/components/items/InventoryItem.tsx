...
import {useAuth, useEquippedItems, useHasModifiers} from '../../store/selectors';
import {
    Card,
    CardContent,
    CardHeader,
...
const InventoryItem: FunctionComponent<InventoryItemProps> = (props: InventoryItemProps) => {
    ...
    useFirebaseConnect([
        {
            path: `/items/${character}/${item}`,
            storeAs: `/inventoryItem/${item}`,
        }, {
            path: `/characters/${character}/inventory`,
            storeAs: `/inventoryItem/${item}/inventory`,
        }, {
        ...
        },
        {
            ...
            queryParams: [`orderByChild=parent`, `equalTo=${item}`],
            storeAs: `itemModifiers/${item}`
        },
    ]);
    const itemData: Item = useTypedSelector(state => state.firebase.data?.inventoryItem?.[item]);
    const equipped = useEquippedItems(character);

    const [isEquipped,setIsEquipped] = useState(equipped[item] ?? false);
    ...
    const hasModifiers = useHasModifiers(item);
    ...
    const handleChange: FormValueChangeHandler = (updates) => {
        ...
        const update = updates.reduce<Partial<Item>>((prev, curr) => {
            const sizeOverride = calculateSize(itemData, curr);
        ...
        firebase.ref(`/items/${character}/${item}`)
            .update(update);
    };

    useEffect(()=>{
        setIsEquipped(equipped[item] ?? false);
    },[equipped[item]])

    const handleEquip = (event: React.ChangeEvent<HTMLInputElement>) => {
        setIsEquipped(event.target.checked ?? false);
        firebase.ref(`/equippedItems/${character}`).update({[item]: event.target.checked});
    };

    if (!isLoaded(itemData)) return <Card><CardContent>Loading</CardContent></Card>;
                    ...
                    <Card className={classes.card}>
                        ...
                        <CardActions className={classes.cardActions}>
                            <div className={classes.sectionToggles}>
                                ...
                                <SectionToggleIcon
                                    ...
                                    show={hasModifiers || expanded.sectionToggles}
                        ...
                        <CollapsingSection expand={expanded.isExpanded}>
                            ...
                            <FadingSection
                                onClose={toggle}
                                expanded={expanded.modifiers}>
                                <ModifierSection sectionId={"hasModifiers"} onChange={handleChange}/>
                            </FadingSection>
                            <FadingSection
                                onClose={toggle}
                                expanded={expanded.settings}>
