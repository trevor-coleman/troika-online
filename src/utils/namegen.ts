...
export class Generator {
  ...
  constructor(pattern: string | Generator[] | null = null,
              collapse_triples: boolean = true)
  {
    ...
    for (let i = 0; i < patternStr.length; i++) {
      ...
      switch (chr) {
        ...
        case ')':
          ...
          last = top.produce();
          top = stack.pop() ?? new Group(GroupTypes.symbol);
          top.add(last ?? "");
          break;
        case '|':
          top.split();
    ...
    let g: Generator = top.produce() ?? new Generator('');
    if (collapse_triples) {
      g = new Collapser(g);
    }
    this.add(g);
